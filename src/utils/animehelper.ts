...
export default async function getM3U8(episodeUrl: string) {
    ...
    const pstream_data = await (await fetch(pstream_url, {
        headers: {
    ...
    const pstream_script = await (await fetch(pstream_script_url, {
        headers: {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36",
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9",
            "Accept-Language": "en-US,en;q=0.9,fr;q=0.8",
...
export async function getStreamtapeMP4(episodeUrl: string) {
    ...
    const streamtape_data = await (await fetch(streamtape_url, {
        headers: {
